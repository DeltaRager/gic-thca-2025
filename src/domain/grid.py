...
class Grid(BaseModel):
    ...
    def add_car(self, id: str, x: int, y: int, direction: Direction, commands: str) -> None:
        if len(self.cars.keys()) >= self.size_x * self.size_y:
            raise ValueError("Cannot add more cars than the grid can hold")
        
        for car_id, car in self.cars.items():
            if car.position == (x, y):
                raise ValueError(f"Position ({x}, {y}) is already occupied by car {car_id}")

        if id in self.cars.keys():
            raise ValueError(f"Car with id '{id}' already exists")

        if not self.is_within_bounds(x, y):
            raise ValueError(f"Car position ({x}, {y}) is out of bounds on grid size {self.size_x}x{self.size_y}")
    ...
    def check_collisions(self) -> bool:
        ...
        for pos, car_ids in positions.items():
            if len(car_ids) > 1:
                self.logger.debug(f"Collision detected at position {pos} between cars: {', '.join(car_ids)}")
    ...
    def next_step(self) -> None:
        for car_id, car in self.cars.items():
            ...
            if command is None:
                logging.getLogger(__name__).debug(f"Car {car_id} has no more commands to execute")
                continue
            new_x, new_y, new_direction = car.calculate_command(command)
            if self.is_within_bounds(new_x, new_y):
                car.move(command)
                logging.getLogger(__name__).debug(f"Executing command {command} for car {car_id} at step {self.current_step}")
                logging.getLogger(__name__).debug(f"Car {car_id} moved to ({new_x}, {new_y}) facing {new_direction}")
            else:
                logging.getLogger(__name__).warning(f"Car {car_id} cannot move to ({new_x}, {new_y}) - out of bounds")
