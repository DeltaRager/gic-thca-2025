...
def main():
    ...
    while i < len(lines):
        ...
        if i >= len(lines):
            print(f"ERROR: Car {car_number} ('{car_id}') missing position and direction")
            show_format_help()
            sys.exit(1)
        ...
        try:
            ...
            print(f"ERROR: Car {car_number} ('{car_id}') has invalid position/direction: '{lines[i]}'")
        ...
        if i < len(lines) and lines[i]:
            ...
            if not is_next_car:
                ...
                for cmd in commands:
                    if cmd not in 'FLR':
                        print(f"ERROR: Car {car_number} ('{car_id}') has invalid command '{cmd}' in '{commands}'")
                        print("Commands must contain only F (Forward), L (Left), R (Right)")
                        show_format_help()
                        sys.exit(1)
                i += 1
